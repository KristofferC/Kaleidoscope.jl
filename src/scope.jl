...
struct Scope
    values::Dict{String, LLVM.AllocaInst}
    parent::Scope

    Scope() = new(Dict{String, LLVM.AllocaInst}())
    Scope(parent::Scope) = new(Dict{String, LLVM.AllocaInst}(), parent)
end
...
Base.setindex!(currentscope::CurrentScope, v::LLVM.AllocaInst, var::String) = currentscope.scope.values[var] = v
